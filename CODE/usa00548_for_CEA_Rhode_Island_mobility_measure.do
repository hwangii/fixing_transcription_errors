...
quietly infix               ///
  ...
  long    bpld       19-23  ///
  byte    migrate5   24-24  ///
  byte    migrate5d  25-26  ///
  str     histid     27-62  ///
  using `"`original_d'usa_00548.dat"'
...
drop migrate5d
gen lifetime_mig=bpl!=statefip if bpl<=56 & statefip<=56 & bpl!=. & statefip!=.
gen fiveyr_st_mig=migrate5==3|migrate5==4 if migrate5>=1 & migrate<=4
gen fiveyr_ct_mig=migrate5==2|migrate5==3|migrate5==4 if migrate5>=1 & migrate<=4

keep histid lifetime_mig fiveyr_st_mig fiveyr_ct_mig age sex

save `"`original_d'rhode_island_mobility1940.dta"',replace
